...
const kLinesBuffer = Symbol('kLinesBuffer');

function InterfaceConstructor(input, output, completer, terminal, forceConversationMode) {
  this[kSawReturnAt] = 0;
  ...
  if (input?.input) {
    ...
    forceConversationMode = input.forceConversationMode;
  ...
  this.forceConversationMode = forceConversationMode;
...
class Interface extends InterfaceConstructor {
  ...
  [kQuestion](query, cb) {
    ...
    if (this[kQuestionCallback]) {
      ...
      this.prompt();
      if(this.forceConversationMode && this[kLinesBuffer].length > 0) {
        const bufferedLine = this[kLinesBuffer].shift();
        this[kOnLine](bufferedLine);
      }
    }
  }
  ...
  [kOnLine](line) {
    if (this[kQuestionCallback]) {
      ...
      if(this.forceConversationMode) {
        this[kLinesBuffer].push(line);
      } else {
        this.emit('line', line);
      }
    }
